...
public class S2CGuildSyncPacket {
    ...
    public S2CGuildSyncPacket(FriendlyByteBuf buf) {
        ...
        if (inGuild) {
            ...
            this.members      = readList(buf, b -> new MemberEntry(b.readUUID(), b.readUtf(32), b.readBoolean(), b.readUtf(16)));
            ...
            this.guildName = null; this.ownerUUID = null;
            this.motd = ""; this.description = "";
            this.friendlyFire = false; this.homeSet = false;
        ...
        this.allGuilds = readList(buf, b -> new GuildSummary(b.readUtf(64), b.readVarInt(), b.readVarInt(), b.readUtf(256)));
    }
    ...
    public void encode(FriendlyByteBuf buf) {
        ...
        if (inGuild) {
            ...
            writeList(buf, members,  (b, m) -> { b.writeUUID(m.uuid()); b.writeUtf(m.name(), 32); b.writeBoolean(m.isOnline()); b.writeUtf(m.rank(), 16); });
            writeList(buf, allies,   (b, a) -> { b.writeUtf(a.name(), 64); b.writeVarInt(a.memberCount()); b.writeVarInt(a.onlineCount()); });
            writeList(buf, pendingOutgoing, (b, p) -> b.writeUtf(p.guildName(), 64));
            writeList(buf, pendingIncoming, (b, p) -> b.writeUtf(p.guildName(), 64));
            writeList(buf, logEntries, (b, l) -> { b.writeLong(l.timestamp()); b.writeUtf(l.message(), 256); });
            writeList(buf, wikiPages,  (b, w) -> { b.writeUtf(w.title(), 64); b.writeUtf(w.content(), 512); });
        }
        writeList(buf, allGuilds, (b, g) -> { b.writeUtf(g.name(), 64); b.writeVarInt(g.memberCount()); b.writeVarInt(g.onlineCount()); b.writeUtf(g.description(), 256); });
    }
    ...
    @FunctionalInterface private interface ElemReader<T> { T read(FriendlyByteBuf b); }
    @FunctionalInterface private interface ElemWriter<T> { void write(FriendlyByteBuf b, T t); }

    private static <T> List<T> readList(FriendlyByteBuf buf, ElemReader<T> r) {
        int n = buf.readVarInt(); List<T> out = new ArrayList<>(n);
        for (int i = 0; i < n; i++) out.add(r.read(buf));
        return out;
    }
    private static <T> void writeList(FriendlyByteBuf buf, List<T> list, ElemWriter<T> w) {
        buf.writeVarInt(list.size()); for (T t : list) w.write(buf, t);
    }
}
