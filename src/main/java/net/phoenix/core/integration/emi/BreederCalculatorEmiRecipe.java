...
public class BreederCalculatorEmiRecipe implements EmiRecipe {
    ...
    public ResourceLocation getId() {
        return new ResourceLocation("phoenixcore", "breeder_calculator");
    }
    ...
    public void addWidgets(WidgetHolder widgets) {
        ...
        addStepButtons(widgets, 28, rowY - 1,
                v -> {
                    rodCount = Math.max(1, rodCount + v);
                    EmiApi.displayRecipe(this);
                });
        widgets.addText(Component.literal("×" + rodCount), 46, rowY + 5, 0xFFFFFF, false);
        widgets.addText(Component.literal(fuel.getName()).withStyle(ChatFormatting.WHITE), 68, rowY + 1, 0xFFFFFF,
                false);
        widgets.addText(
        ...
        widgets.addText(Component.literal(blanket.getName()).withStyle(ChatFormatting.WHITE), 28, rowY + 1, 0xFFFFFF,
                false);
        widgets.addText(
                Component.literal(durationSec + "s · " + blanket.getAmountPerCycle() + "/cycle")
        ...
        // ── ROW: Moderator ────────────────────────────────────────────────────
        rowY = y;
        widgets.addSlot(FuelRodEmiRecipe.getEmiStackFromId("phoenixcore:" + moderator.getName()), 6, rowY)
                .drawBack(true);
        widgets.addButton(6, rowY, 18, 18, 0, 0, () -> true, (mx, my, b) -> {
            moderatorIndex = (moderatorIndex + 1) % FissionModeratorBlock.FissionModeratorTypes.values().length;
            EmiApi.displayRecipe(this);
        });
        widgets.addText(Component.literal(moderator.getName()).withStyle(ChatFormatting.WHITE), 28, rowY + 1, 0xFFFFFF,
                false);
        widgets.addText(
        ...
        addStepButtons(widgets, 28, rowY - 1,
                v -> {
                    burnTimeMin = Math.max(0, Math.min(BURN_RAMP_MIN, burnTimeMin + v));
                    EmiApi.displayRecipe(this);
                });
        ...
        for (int i = 0; i < outputs.size(); i++) {
            ...
            widgets.addSlot(FuelRodEmiRecipe.getEmiStackFromId(o.key()), slotX, y)
                    ...
                    .appendTooltip(
                            Component.literal(String.format("%.1f%%", pct) + " chance").withStyle(ChatFormatting.WHITE))
                    .appendTooltip(
                            Component.literal(String.format("~%.1f expected", exp)).withStyle(ChatFormatting.YELLOW))
        ...
        // Totals row
        widgets.addText(
                Component.literal("~" + amount + " rolls · " + parallels + " parallels")
