...
public class FissionCalculatorEmiRecipe implements EmiRecipe {
    ...
    @Override public ResourceLocation getId() { return new ResourceLocation("phoenixcore", "fission_calculator"); }
    ...
    public int getDisplayHeight() {
        // header(13) + controlRows(3×22=66) + separator(6) + tableHeader(10) + tableRows(6×11=66) + status(22) + footer(8)
    ...
    public void addWidgets(WidgetHolder widgets) {
        ...
        widgets.addDrawable(COL_SEP, 0, 1, getDisplayHeight() - 8, (gui, mx, my, dt) ->
                gui.fill(0, 0, 1, getDisplayHeight() - 8, 0x44AAAAAA));
        ...
        stepButtons(widgets, slotX + 22, ctrlY + 1,
                val -> { fuelCount = Math.max(1, fuelCount + val); EmiApi.displayRecipe(this); });
        ...
        stepButtons(widgets, slotX + 22, ctrlY + 1,
                val -> { coolerCount = Math.max(0, coolerCount + val); EmiApi.displayRecipe(this); });

        String coolerDesc = coolerCount == 0 ? "No cooler" :
                "×" + coolerCount + "  −" + formatHeat(totalCool) + " HU/t";
        widgets.addText(Component.literal(coolerDesc).withStyle(ChatFormatting.WHITE), slotX + 38, ctrlY + 5, 0xFFFFFF, false);
        ...
        stepButtons(widgets, slotX + 2, ctrlY,
                val -> { burnTimeMin = Math.max(0, Math.min(20, burnTimeMin + val)); EmiApi.displayRecipe(this); });
        ...
        widgets.addText(Component.literal(burnTimeMin == 0 ? "no ramp bonus" : "+" + String.format("%.0f", (burnMult - 1) * 100) + "% heat & EU")
        ...
        for (int heat : HEAT_LEVELS) {
            ...
            ChatFormatting netColor = net > 0 ? ChatFormatting.GREEN : net < 0 ? ChatFormatting.RED : ChatFormatting.YELLOW;
            ...
            widgets.addText(Component.literal(
                            ...
                            .append(Component.literal("  " + formatHeatShort((int) Math.round(euPerTick))).withStyle(ChatFormatting.GOLD)),
        ...
        if (meltLevel == 0) {
            ...
            statusText(widgets, "⚠ FEEDBACK LOOP — melt on start", ChatFormatting.RED, ChatFormatting.DARK_RED, slotX, ctrlY);
        } else if (!everStable) {
            statusText(widgets, "⚠ DANGER — cooling never sufficient", ChatFormatting.RED, ChatFormatting.DARK_RED, slotX, ctrlY);
        } else if (meltLevel > 0) {
            statusText(widgets, "⚡ CEILING: unstable above " + formatHeatShort(meltLevel) + " HU", ChatFormatting.GOLD, ChatFormatting.YELLOW, slotX, ctrlY);
        } else {
            statusText(widgets, "✔ STABLE — safe across all heat levels", ChatFormatting.GREEN, ChatFormatting.DARK_GREEN, slotX, ctrlY);
        }
        ...
        for (int mi = 1; mi < modTypes.length; mi++) { // skip index 0 = None
            ...
            // Count + stat summary
            widgets.addText(Component.literal("×" + cnt).withStyle(cnt > 0 ? ChatFormatting.WHITE : ChatFormatting.DARK_GRAY),
        ...
        modY += 10;
        widgets.addText(Component.literal("EU boost: +" + euBoost + "%" + (totalEuPct > MAX_EU_BOOST ? " (capped)" : ""))
                .withStyle(ChatFormatting.GOLD), RIGHT_X, modY, 0xFFFFFF, false);
        modY += 10;
        widgets.addText(Component.literal("Fuel disc: −" + disc + "%" + (totalDiscPct > MAX_FUEL_DISC ? " (capped)" : ""))
                .withStyle(ChatFormatting.GREEN), RIGHT_X, modY, 0xFFFFFF, false);
