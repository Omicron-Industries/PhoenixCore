package net.phoenix.core.conflux.tools;
...
public class AxiomSpriteExporter {
    ...
    public static void main(String[] args) throws IOException {
        String root = args.length > 0 ? args[0]
            : "src/main/resources/assets/phoenixcore/textures/gui/axiom";
    ...
    static int lerp(int a, int b, float t) { return a+(int)((b-a)*t); }
    ...
    static void bakeVoidDisk(File dir) throws IOException {
        ...
        gs.setComposite(AlphaComposite.Clear); gs.fillRect(0,0,sheet.getWidth(),S);
        gs.setComposite(AlphaComposite.SrcOver); gs.dispose();
        ...
        for (int f = 0; f < FRAMES; f++) {
            ...
            for (int layer = 13; layer >= 0; layer--) {
                ...
                RadialGradientPaint rg = new RadialGradientPaint(CX,CY,r,
                    new float[]{0f,1f},
                    new Color[]{new Color(heatCols[layer].getRed(),heatCols[layer].getGreen(),heatCols[layer].getBlue(),alphas[layer]),
                                new Color(0,0,0,0)});
                g.setPaint(rg);
    ...
    static void bakeSculkVeins(File dir) throws IOException {
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                ...
                float bx, by;
                switch (di) {
                    case 0  -> { bx=ax+VEIN_GRID+rng.nextFloat()*40-20; by=ay+rng.nextFloat()*30-15; }
                    case 1  -> { bx=ax+rng.nextFloat()*30-15; by=ay+VEIN_GRID+rng.nextFloat()*40-20; }
                    case 2  -> { bx=ax-VEIN_GRID*0.5f+rng.nextFloat()*30; by=ay+VEIN_GRID*0.6f+rng.nextFloat()*30; }
                    default -> { bx=ax+VEIN_GRID*0.7f+rng.nextFloat()*30; by=ay+VEIN_GRID*0.5f+rng.nextFloat()*20; }
                }
                float span = Math.max(0.01f,(float)Math.sqrt((bx-ax)*(bx-ax)+(by-ay)*(by-ay)));
                float px=-(by-ay)/span*(20+rng.nextFloat()*30), py=(bx-ax)/span*(20+rng.nextFloat()*30);
