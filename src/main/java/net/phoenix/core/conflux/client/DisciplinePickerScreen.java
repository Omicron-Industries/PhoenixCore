...
import net.phoenix.core.conflux.client.render.MotionClock;
...
public class DisciplinePickerScreen extends Screen {
    ...
    private void renderCard(GuiGraphics g, DisciplineCard card, int idx,
        ...
        if (reveal > 0.15f) {
            ...
            if (reveal > 0.45f && sc > 0.7f) {
                ...
                for (int li = 0; li < Math.min(card.loreLines.length, maxLines); li++) {
                    ...
                    if (line.isEmpty()) { textY += (int)(5 * sc); continue; }
        ...
        if (hov || sel) {
            ...
            int btnBg = sel ? (card.accentColor & 0x00FFFFFF | 0x88000000)
                            : btnHov ? (card.accentColor & 0x00FFFFFF | 0x66000000)
                                     : (card.accentColor & 0x00FFFFFF | 0x33000000);
            ...
            g.drawString(font, btnLabel, cx - blw / 2, btnY + (btnH - 8) / 2, sel ? (card.accentColor | 0xFF000000) : 0xFFCCCCCC, false);
        }
    }
    ...
    private void renderConfirmOverlay(GuiGraphics g, int mx, int my) {
        ...
        String[] confirmLines = card.sealed
            ? new String[]{"This path is sealed.", "Committing to it may have consequences", "beyond what this terminal can show you."}
            : new String[]{"You are about to choose " + card.name + ".", "This choice can be changed later,", "but not without cost.", "Are you certain?"};
    ...
    public boolean keyPressed(int kc, int sc2, int mod) {
        ...
        if (kc == GLFW.GLFW_KEY_LEFT  || kc == GLFW.GLFW_KEY_A) { arcVelocity -= 600f; return true; }
        if (kc == GLFW.GLFW_KEY_RIGHT || kc == GLFW.GLFW_KEY_D) { arcVelocity += 600f; return true; }
    ...
    public boolean mouseDragged(double mx, double my, int btn, double dx, double dy) {
        if (btn == 0) { arcOffset += (float)dx * 1.5f; return true; }
        ...
        if (minecraft != null && minecraft.player != null) {
            minecraft.player.displayClientMessage(
                Component.literal("§6[Axiom] §7Path chosen: §r" + card.name +
                    "§7. Costs will be applied in a future update."), false);
        }
        onClose();
    }
